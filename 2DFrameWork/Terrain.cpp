...
void Terrain::LoadHeightImage(string file)
{
	...
	else if (img->GetImages()[0].format == DXGI_FORMAT_R32_FLOAT)
	{
		float* data = (float*)img->GetImages()[0].pixels;
		for (int i = 0; i < garo; i++)
		{
			for (int j = 0; j < garo; j++)
			{
				VertexTerrain* vertices = (VertexTerrain*)mesh->vertices;
				float _y = data[i * garo + j]; // Assuming that data is directly in float format.
				vertices[i * garo + j].position.y = _y;
			}
		}
	}
	...
	else if (img->GetImages()[0].format == DXGI_FORMAT_R8G8B8A8_UNORM)
	{
		...
		for (int i = 0; i < garo; i++)
		{
			...
			{
				VertexTerrain* vertices = (VertexTerrain*)mesh->vertices;
				int dataIndex = (i * garo + j) * 3; // Each pixel consists of 3 bytes (R, G, B)

				// Calculate the height from the RGB values (you may need to adjust this depending on your data)
				float _y = (float)(data[dataIndex] + data[dataIndex + 1] + data[dataIndex + 2]) / (3.0f * 255.0f);
